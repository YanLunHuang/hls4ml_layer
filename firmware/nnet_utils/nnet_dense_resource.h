...
void dense_large_rf_gt_nin_rem0(
    data_T data[CONFIG_T::n_in],
    res_T  acc[CONFIG_T::n_out/CONFIG_T::reuse_factor][CONFIG_T::reuse_factor],
    typename CONFIG_T::weight_t weights[CONFIG_T::n_in*CONFIG_T::n_out],
    typename CONFIG_T::bias_t   biases[CONFIG_T::n_out]) {
    
    const int block_factor = DIV_ROUNDUP(CONFIG_T::n_out,CONFIG_T::reuse_factor);
    
    #pragma HLS function_instantiate variable=biases,weights
    ...
    #pragma HLS ARRAY_PARTITION variable=biases complete
     ...
     for(int i_in = 0; i_in < CONFIG_T::n_in; i_in++) {
		...
		data_T tmpt = data[i_in];
        for (int iacc = 0; iacc < CONFIG_T::reuse_factor; iacc++) {
          #pragma HLS UNROLL
          for (int iacc2 = 0; iacc2 < block_factor; iacc2++) {
            #pragma HLS UNROLL
            unsigned w_index  =  i_in + (CONFIG_T::n_in*(iacc*block_factor+iacc2)); 
            acc[iacc2][iacc] += product_dense<data_T, typename CONFIG_T::weight_t, typename CONFIG_T::accum_t>(tmpt, weights[w_index]);
          }
        }
     }
}
