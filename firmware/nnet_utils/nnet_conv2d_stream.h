...
void conv_2d_large_cl_nopad_pad_me(
    ...
    const int block_factor = DIV_ROUNDUP(CONFIG_T::n_filt,CONFIG_T::reuse_factor);
    #pragma HLS ARRAY_RESHAPE variable=layer_in block factor=CONFIG_T::n_chan

    res_T layer_out[block_factor][CONFIG_T::reuse_factor];
    ...
    for (unsigned i = 0; i < CONFIG_T::in_height * CONFIG_T::in_width; i++) {
        ...
        if ( (sX - lShiftX) == 0 && (sY - lShiftY) == 0 && pY > lShiftY - 1 && pX > lShiftX - 1) {
            ...
            if (CONFIG_T::strategy == nnet::latency) {
                dense_large_rf_gt_nin_rem0<data_T, res_T, typename CONFIG_T::mult_config>(layer_in, layer_out, weights, biases);
            } else {
                dense_large_rf_gt_nin_rem0<data_T, res_T, typename CONFIG_T::mult_config>(layer_in, layer_out, weights, biases);
            }
            ...
            ResWrite:for (int iacc = 0; iacc < CONFIG_T::reuse_factor; iacc++) {
                ...
                for (int iacc2 = 0; iacc2 < block_factor; iacc2++) {
                    #pragma HLS UNROLL
                    res_T tmpres = layer_out[iacc2][iacc];
                    res.write(tmpres);
                }
            }
        }
